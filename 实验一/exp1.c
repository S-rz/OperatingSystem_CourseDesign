...
{
    int filesource,filetarget;//文件指针
    if(argc!=4)
    {
      printf("Argc Error!\n");
      return 0;
    ...
    if( (filesource=open(argv[2],O_RDONLY))==-1 )
    {
      printf("pen Source File Error!\n");
      return 0;
    }
    if( (filetarget=open(argv[3],O_WRONLY|O_CREAT,S_IRWXU|S_IXGRP|S_IROTH|S_IXOTH))==-1 )
    {
    ...
    if((pid1=fork())<0)
    {
  	...
  	else if(pid1==0)writebuf(filesource);
   	else {
  		if((pid2=fork())<0){
  			perror("Fail to fork");
  			exit(EXIT_FAILURE);
  		}
  		else if(pid2==0)readbuf(filetarget);
  		else
  		{
    			/***等待子进程结束***/
    			waitpid(pid1,NULL,0);
